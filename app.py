...
def votar():
    """Registar um voto de satisfação no Firebase"""
    try:
        if not firebase_initialized:
            return jsonify({"erro": "Base de dados não configurada"}), 500
        ...
        # Salvar com timestamp como chave única
...
def dashboard():
    """Dashboard com estatísticas do Firebase"""
    if not session.get("admin"):
        return redirect(url_for("login_admin"))
    
    if not firebase_initialized:
        return render_template("admin__dashboard.html", erro="Base de dados não configurada")
